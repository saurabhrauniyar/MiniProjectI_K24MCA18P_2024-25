...
from .maze import GOAL, START, Maze
...
def draw() -> None:
    ...
    for text in texts:
        ...
        if text == "Start Node":
            image_rect = START.get_rect(center=(x-200 , y + 15))  # Align with Start Node rectangle
            WINDOW.blit(START, image_rect)
        elif text == "Target Node":
            # Center the image inside the rectangle for the Target Node
            image_rect = GOAL.get_rect(center=(x - 210, y + 15))  # Align with Target Node rectangle
